...
int main()
{
    ...
    while (1)
    {
        ...
        if (sz && mem->platformStatus[0] == 0)
        {
            ...
            if (mem->platformStatus[0] != 0 && mem->platformStatus[1] == 0)
            {
                cout << "I am free! So I will accept the train\n";
                int nsfd = 0;
                cout << getFD(usfd) << "\n";
                cout << nsfd << "\n";
                mem->platformStatus[1] = 1;
                cout << "Connection recieved: " << nsfd << "\n";
                readFromTrain(nsfd);
                informSM(usfd);
            }
            else
    ...
    // while (1)
    // {
    //     int sigRecieved = recieveSignal();
    //     if (signalPid == stationMaster)
    //     {
    //         cout << "\nMessage recieved from station master!";
    //         break;
    //     }
    //     else
    //     {
