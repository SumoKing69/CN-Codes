...
int main(){
    ...
    while(1){
        if(sp_rem->chance == 0){
            int type_id = sp_rem->recv_message;
            printf("type_id is %d\n",type_id);
            int nsfd = recv_fd(usfd);
            for(int i=0;i<5;i++){ 
                // Reading from message queue!
                int gf = msgrcv(msgid,&msg_msg,sizeof(msg_msg.msg_text),1,0);  int ans = 0;
                ...
                if(ans == 0){
                    printf("Received from SP1 %s\n",msg_msg.msg_text);
                }
            }
            sp_rem->chance = 100;
        }
        else if(sp_rem->chance == 1){
            printf("SP : I am speaking!\n");
